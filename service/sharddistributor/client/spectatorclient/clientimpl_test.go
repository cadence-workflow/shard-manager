...
// A drained shard must be refused from local state. The mock client has no
// GetShardOwner expectation, so any RPC fallback fails these cases.
func TestGetShardOwner_DrainedShard(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// The leader has not dropped the assignment yet. The shard distributor
			// refuses a drained shard even while it has an owner, so we must agree.
...
// Each response is a full snapshot, so an undrain is just a snapshot without the
// shard. The spectator must stop refusing it and fall back to an RPC again.
